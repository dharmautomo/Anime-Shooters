...
function AnimePlayerModel({ isMainPlayer, position, rotation, health, username }: AnimePlayerProps) {
  ...
  useEffect(() => {
    ...
    if (groupRef.current && scene) {
      try {
        ...
        const modelScaleFactor = 0.005; // Increased scale to ensure player is visible
        ...
        clonedScene.position.set(0, -0.95, 0); // Adjusted height for larger model
  ...
  useFrame((state, delta) => {
    ...
    if (isMainPlayer && groupRef.current) {
      ...
      if (nameTagRef.current) {
        nameTagRef.current.position.set(0, 2.0, 0); // Raised for larger model
      }
    ...
    >
      ...
      {!isMainPlayer && (
        <SciFiGun 
          ...
          scale={0.0015} // Scaled to match the larger anime character
        />
      )}
    </group>
  );
...
function ModelAttribution() {
  return (
    <sprite position={[0, 2.5, 0]} scale={[2.0, 0.4, 1]}>
      <spriteMaterial 
        transparent={true}
        depthTest={false}
        map={(() => {
